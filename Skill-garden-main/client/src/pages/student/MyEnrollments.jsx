...
const MyEnrollments = () => {
  ...
  <div className='pt-10'>
    ...
    <table className="md:table-auto table-fixed w-full overflow-hidden border border-white/10 mt-10 text-white">
      <thead className="text-gray-300 border-b border-white/10 text-sm text-left max-sm:hidden">
        <tr>
        ...
        {enrolledCourses.map((course, index) => (
          <tr key={index} className="border-b border-white/10">
            ...
            <td className="px-4 py-3 max-sm:hidden">
              ...
              <span className='text-xs ml-2 text-gray-400'>Lectures</span>
            </td>

            <td className="px-4 py-3 max-sm:text-right">
              <button
                onClick={() => navigate('/player/' + course._id)}
                className='px-3 sm:px-5 py-1.5 sm:py-2 bg-teal-500 hover:bg-teal-600 text-white rounded max-sm:text-xs'
              >
                {progressArray[index] &&
